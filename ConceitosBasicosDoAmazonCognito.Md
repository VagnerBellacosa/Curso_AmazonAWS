...
Esta seção descreve as principais tarefas do Amazon Cognito e por onde começar. Para uma visão geral do Amazon Cognito, consulteO que é o Amazon Cognito?.

Os dois componentes principais do Amazon Cognito são os grupos de usuários e os grupos de identidades. Os grupos de usuários são diretórios de usuários que fornecem opções de cadastro e login para os usuários do seu aplicativo Web e móvel. Os grupos de identidade fornecem credenciais da AWS para conceder aos usuários acesso a outros serviços da AWS. Você pode usar grupos de usuários e grupos de identidades separadamente ou em conjunto.
